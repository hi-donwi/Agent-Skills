...
**NOT for:**
- Diagnosing a failure happening right now — use the `debugging` skill (observability is what makes that skill fast next time)
...
**Never log secrets, tokens, passwords, or full PII.** This is a hard rule from the `security-hardening` skill — telemetry pipelines are a classic data-leak path. Allowlist fields; don't log whole request bodies.
