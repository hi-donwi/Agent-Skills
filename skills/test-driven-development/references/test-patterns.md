...
Load this when writing tests, choosing a test size, or reviewing a flaky or over-mocked suite.
...
With tests green, extract shared logic, improve naming, remove duplication, and optimize only if necessary. Run tests after every refactor step.

## Prove-it (bug fixes)

```
Bug report arrives → write a failing reproduction test → implement the fix
...
For anything that runs in a browser, unit tests alone aren't enough. Use Chrome DevTools MCP for DOM inspection, console logs, network requests, performance traces, and screenshots.
...
See `webapp-testing/references/devtools-mcp.md` for setup.

## Subagents for reproduction tests

For complex bug fixes, spawn a subagent to write the reproduction test without knowledge of the fix. The main agent verifies the test fails, implements the fix, then verifies the test passes.
